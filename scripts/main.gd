extends Node2D
## Stage-C: suika feel + Miku weighted spawn / chain score / cyan theme.
...
func _ready() -> void:
	...
	_danger.width = 2.0
	_aim.width = 1.5
	_aim.default_color = Color(0.45, 0.95, 0.9, 0.35)
...
func _add_static_rect(pos: Vector2, size: Vector2) -> void:
	...
	vis.color = Color(0.15, 0.32, 0.34)
	body.add_child(vis)
...
func _update_preview() -> void:
	...
	var nr := FusionData.radius(_next_lv) * 0.45
	_next_preview.color = FusionData.color(_next_lv)
...
func _spawn_fruit(lv: int, pos: Vector2) -> FusionOrb:
	var fruit: FusionOrb = _fruit_scene.instantiate() as FusionOrb
...
func _on_merge_wanted(a: RigidBody2D, b: RigidBody2D) -> void:
	...
	if a == null or b == null or not is_instance_valid(a) or not is_instance_valid(b):
		return
	var fa := a as FusionOrb
	var fb := b as FusionOrb
	if fa == null or fb == null:
		return
	if fa.merging or fb.merging:
		return
	if fa.level != fb.level or fa.level >= FusionData.MAX_LV:
		return
	fa.merging = true
	fb.merging = true
	var mid := (fa.global_position + fb.global_position) * 0.5
	var new_lv: int = fa.level + 1
	var add_score: int = _next_merge_score(new_lv)
	...
	_score += add_score
	_float_score(mid + Vector2(0, -FusionData.radius(new_lv)), "+%d" % add_score)
	_update_hud()


func _next_merge_score(new_lv: int) -> int:
	var now := Time.get_ticks_msec()
	if now <= _chain_until_msec:
		_chain_index += 1
	else:
		_chain_index = 1
	_chain_until_msec = now + 450
	return FusionData.merge_score(new_lv, _chain_index)
